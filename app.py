...
def convert_Pokemon_from_db(a_pokemon_db):
    if type(a_pokemon_db) == str:
        return Pokemon(a_pokemon_db, '', '', 0, 0, 0, 0, 0, 0, 0)
    else:
        a_pokemon = Pokemon(
            a_pokemon_db.get_name(),
            a_pokemon_db.get_principalType(),
            a_pokemon_db.get_secondaryType(),
            a_pokemon_db.get_attack(),
            a_pokemon_db.get_defense(),
            a_pokemon_db.get_sp_attack(),
            a_pokemon_db.get_sp_defense(),
            a_pokemon_db.get_speed(),
            a_pokemon_db.get_hp(),
            a_pokemon_db.get_total()
        )
        return a_pokemon
...
def create_app():
    ...
    def next_turn():
        try:
            ...
            battle = Fight(fighter_one, fighter_two)
